...
export default function ContactPage() {
  return (
    <div className="mx-auto max-w-2xl px-5 md:px-6 py-10">
      <h1 className="text-3xl font-semibold text-yuri-ink mb-4">
        お問い合わせ
      </h1>
      <p className="text-base md:text-lg text-yuri-ink/90 leading-relaxed mb-8">
        ご質問・情報提供・ご相談など、お気軽にどうぞ。
        <br />
        <strong className="font-semibold text-[#06C755]">いちばん早いのは LINE です。</strong>
      </p>
      ...
      {LINE_URL && (
        <div className="rounded-xl border-2 border-[#06C755]/50 bg-gradient-to-br from-[#06C755]/15 to-yuri-cream p-6 md:p-7 mb-6 text-center">
          ...
          <h2 className="text-xl font-semibold text-yuri-ink mb-2">
            LINE でお問い合わせ
          </h2>
          <p className="text-base text-yuri-ink/85 leading-relaxed mb-5">
            友だち追加して送るだけ。いちばんスムーズです。
          </p>
          <a
            href={LINE_URL}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center justify-center gap-2 bg-[#06C755] text-white px-8 py-4 rounded-full text-lg font-semibold hover:opacity-90 transition-opacity shadow-sm"
          >
      ...
      {/* 誹謗中傷NG */}
      <div className="bg-yuri-rose/10 border border-yuri-rose/30 rounded-md p-5 mb-8 text-sm text-yuri-ink/85 leading-relaxed">
        <p className="font-semibold text-yuri-rose mb-2">
          ⚠ ご利用上の注意
        </p>
        <p>
      ...
      <details className="group border-t border-yuri-edge pt-6">
        <summary className="cursor-pointer list-none flex items-center gap-2 text-base text-yuri-muted hover:text-yuri-ink select-none">
          <span className="text-yuri-rose transition-transform group-open:rotate-90">▶</span>
          メールフォームはこちら
        </summary>
        <p className="text-sm text-yuri-muted leading-relaxed mt-3 mb-6">
          スポンサー・タイアップなど、記録を残したい正式なご相談はこちら。3〜5営業日以内にお返事します。
        </p>
